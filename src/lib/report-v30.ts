...
export interface V30PrivateAuditEnvelope {
  rawAudit?: V30Audit;
  elements: Pick<
    NormalizedV30Element,
    "id" | "evidence" | "usedUcs" | "audit"
  >[];
}
...
const asObject = (value: unknown): ObjectRecord =>
...
function normalizeFallback(
  ...
  if (!isObject(value) || typeof value.used !== "boolean") {
    addError(
      ...
      path,
      "invalid_fallback",
      "fallback.used deve ser booleano explícito.",
    );
    return { used: false };
  }
  if (
    ...
    addError(
      errors,
      path,
      "fallback_origin_required",
      "Fallback usado exige source ou reason.",
    );
  }
  return {
    used: value.used,
...
function validateScores(
  ...
  for (const key of ["assertivo", "participativo", "integrador", "analitico"]) {
    ...
    if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
      addError(
        errors,
        `report_output.resultado_calculado.scores.${key}`,
        "invalid_score",
        "Score obrigatório deve ser numérico, finito e igual ou superior a zero.",
      );
    }
  }
}
...
function buildNormalized(
  ...
  const enabled = V30_REPORT_FIELDS.map((key) => elements[key]).filter(
    (element) => element.enabled,
  );
  const generated = enabled.filter(
    (element) => element.status === "generated" && element.content !== null,
  ).length;
  const reportStatus: Exclude<V30ReportStatus, "invalid"> =
    generated === enabled.length
      ? "complete"
      : generated === 0
        ? "failed"
        : "partial";
  ...
  return {
    contractVersion: "V30",
    workflowVersion: isNonEmptyString(root.workflow_version)
      ? root.workflow_version
      : undefined,
    resultadoId,
    relatorioUuid,
    elements,
    ...
    privateAudit: {
      rawAudit: Object.keys(rawAudit).length > 0 ? rawAudit : undefined,
      elements: V30_REPORT_FIELDS.map((key) => {
...
export function validateV30Report(response: unknown): V30ValidationResult {
  ...
  if (Object.keys(output).length === 0)
    addError(
      ...
      "report_output deve existir.",
    );
  const version = getContractVersion(root);
  if (!isCompatibleV30Version(version))
    addError(
      errors,
      "contractVersion",
      "unsupported_contract",
      "contractVersion deve ser compatível com V30.",
    );

  const resultadoIdValue =
    root.resultado_id ??
    root.id_resultado ??
    asObject(root.persistence).resultado_id ??
    asObject(output.identificacao).resultado_id;
  const uuidValue =
    root.relatorio_uuid ??
    asObject(root.persistence).relatorio_uuid ??
    asObject(output.identificacao).relatorio_uuid;
  if (!isNonEmptyString(resultadoIdValue))
    addError(
      errors,
      "resultado_id",
      "invalid_resultado_id",
      "resultado_id deve ser uma string não vazia.",
    );
  if (!isUuid(uuidValue))
    addError(
      errors,
      "relatorio_uuid",
      "invalid_relatorio_uuid",
      "relatorio_uuid deve ser um UUID válido.",
    );
  ...
  for (const key of V30_REPORT_FIELDS) {
    ...
    if (matches.length === 0) {
      addError(
        errors,
        `report_output.campos_relatorio.${key}`,
        "missing_element",
        "Elemento obrigatório ausente.",
      );
      continue;
    }
    if (matches.length > 1 || seen.has(key))
      addError(
        errors,
        `report_output.campos_relatorio.${key}`,
        "duplicate_element",
    ...
    if (candidate.id !== key)
      addError(
        errors,
        `report_output.campos_relatorio.${key}.id`,
        "id_key_mismatch",
        "id deve corresponder à chave do elemento.",
      );
    if (!isNonEmptyString(candidate.titulo ?? candidate.title))
      addError(
        errors,
        `report_output.campos_relatorio.${key}.titulo`,
        "missing_title",
    ...
    if (
      ...
      addError(
        errors,
        `report_output.campos_relatorio.${key}.status`,
        "invalid_status",
    ...
    if (
      ...
      addError(
        errors,
        `report_output.campos_relatorio.${key}.enabled`,
        "invalid_enabled",
        "enabled/habilitado deve ser booleano explícito.",
      );
    const order = candidate.ordem ?? candidate.order;
    if (typeof order !== "number" || !Number.isInteger(order))
      addError(
        errors,
        `report_output.campos_relatorio.${key}.ordem`,
        "invalid_order",
        "ordem deve ser número inteiro.",
      );
    const content = normalizeContent(
      candidate.conteudo ?? candidate.content ?? candidate,
    );
    if (status === "generated" && content === null)
      addError(
        errors,
        `report_output.campos_relatorio.${key}.conteudo`,
        "missing_generated_content",
        "generated exige conteúdo válido e não vazio.",
      );
    const evidence = normalizeCollection(
      candidate.evidencias ?? candidate.evidence,
    );
    const ucs = normalizeCollection(
      candidate.ucs_utilizadas ?? candidate.used_ucs ?? candidate.ucs,
    );
    if (evidence === null)
      addError(
        errors,
        `report_output.campos_relatorio.${key}.evidencias`,
        "invalid_evidence",
        "evidências devem ser uma coleção.",
      );
    if (ucs === null)
      addError(
        errors,
        `report_output.campos_relatorio.${key}.ucs_utilizadas`,
        "invalid_ucs",
        "UCs utilizadas devem ser uma coleção.",
      );
    const fallback = normalizeFallback(
      candidate.fallback,
      `report_output.campos_relatorio.${key}.fallback`,
      errors,
    );
    const audit = candidate.auditoria ?? candidate.audit;
    if (audit !== undefined && !isObject(audit))
      addError(
        errors,
        `report_output.campos_relatorio.${key}.auditoria`,
        "invalid_audit",
        "auditoria deve ser um objeto.",
      );
    elements[key] = {
      id: key,
  ...
  const report = buildNormalized(
    root,
    output,
    elements,
    resultadoIdValue as string,
    uuidValue as string,
  );
...
export function resolveV30Report(response: unknown): {
  declared: boolean;
  validation: V30ValidationResult;
} {
  const root = parseObject(response);
  const nested = parseObject(root.raw_payload);
  const candidate = isV30ReportCandidate(root)
    ? root
    : isV30ReportCandidate(nested)
      ? nested
      : root;
...
export function resolveV30ReportStatus(
  report: NormalizedV30Report | V30ValidationResult,
): V30ReportStatus {
  if ("valid" in report)
    return report.valid
      ? (report.report?.reportStatus ?? "invalid")
      : "invalid";
...
export function v30Scores(report: NormalizedV30Report): Scores | null {
  ...
  const scores = asObject(
    asObject(report.reportOutput.resultado_calculado).scores,
  );
  const values = {
    Assertivo: scores.assertivo ?? scores.Assertivo,
    Participativo: scores.participativo ?? scores.Participativo,
    Integrador: scores.integrador ?? scores.Integrador,
    Analitico: scores.analitico ?? scores.Analitico,
  };
...
export function v30ToResultado(
  report: NormalizedV30Report,
  user: { uid: string; nome: string; empresa_id: string; empresa_nome: string },
): Resultado | null {
  ...
  return {
    ...
    id_usuario: user.uid,
    nome_usuario: isNonEmptyString(identification.nome)
      ? identification.nome
      : user.nome,
    empresa_id: isNonEmptyString(identification.empresa_id)
      ? identification.empresa_id
      : user.empresa_id,
    empresa_nome: isNonEmptyString(identification.empresa)
      ? identification.empresa
      : user.empresa_nome,
    scores,
    data_conclusao: isNonEmptyString(identification.data_conclusao)
      ? identification.data_conclusao
      : "",
    generated_at: isNonEmptyString(identification.generated_at)
      ? identification.generated_at
      : undefined,
    relatorio_pronto_para_app: report.publicReport,
    metadata: {
      contract_version: report.contractVersion,
      workflow_version: report.workflowVersion,
    },
  };
}
