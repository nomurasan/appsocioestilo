...
function contentText(value: unknown): string {
  ...
  if (isV30Content(value)) {
    ...
    if (value.kind === "list")
      return value.items.map(asString).filter(Boolean).join("\n");
...
function contentList(value: unknown, preferredKeys: string[] = []): string[] {
  ...
  if (isV30Content(value)) {
    ...
    if (value.kind === "structured")
      return contentList(value.value, preferredKeys);
    return [];
  }
  if (!isRecord(value)) return [];
...
function getPublicElement(
  report: any,
  fieldId: V30ReportFieldName,
): ObjectRecord {
  const publicReport = getReportOutputPublic(report);
  if (!publicReport) return {};
  return asRecord(
    publicReport.elements.find((element) => element.id === fieldId),
  );
}
...
function buildTextField(
  ...
  const fieldStatus = asString(
    sourceRecord.status,
  ) as ReportTextFieldView["status"];
  const enabled = sourceRecord.enabled !== false;
  const content = sourceRecord.content ?? sourceRecord.conteudo ?? fallback;
  const text =
    contentText(content) || contentText(sourceRecord) || contentText(fallback);
  const status =
  ...
  return {
    ...
    status,
    enabled,
    ...(asString(sourceRecord.style)
      ? { style: asString(sourceRecord.style) }
      : {}),
  };
}
...
function buildRelationsView(
  ...
  const structured = unwrapStructuredObject(
    source.content ?? source.conteudo ?? source,
  );
  const text =
    contentText(
      structured.text ?? structured.texto ?? structured.resumo ?? source,
    ) || contentText(source);
  const combinations = contentList(
    structured.combinacoes_analisadas ??
      structured.combinacoes ??
      structured.relacoes ??
      source.combinacoes_analisadas ??
      source.combinacoes,
    ["combinacoes_analisadas", "combinacoes", "casos", "exemplos"],
  );
  const practicalSituations = contentList(
    structured.situacoes_praticas ??
      structured.situacoes ??
      structured.casos ??
      structured.exemplos ??
      source.situacoes_praticas ??
      source.casos,
    ["situacoes_praticas", "situacoes", "casos", "exemplos"],
  );
  const cautions = contentList(
    structured.cuidados ??
      structured.pontos_de_atencao ??
      source.cuidados ??
      source.pontos_de_atencao,
    ["cuidados", "pontos_de_atencao"],
  );
  const opportunities = contentList(
    structured.oportunidades ?? source.oportunidades,
    ["oportunidades"],
  );
...
function buildRecommendationsView(
  ...
  const list = contentList(
    structured.lista ??
      structured.recomendacoes ??
      source.lista ??
      source.recomendacoes,
    ["lista", "recomendacoes", "items"],
  );
  const firstSteps = contentList(
    structured.primeiros_passos ?? source.primeiros_passos,
    ["primeiros_passos"],
  );

  const talentSource = pickObject(
    structured.potencializacao_talentos,
    source.potencializacao_talentos,
    reportData.potencializacao_talentos,
  );
  const pdiSource = pickObject(
    structured.plano_desenvolvimento_individual,
    structured.pdi,
  ...
  const talentPotentialization: TalentPotentializationView = {
    baseStyle:
      asString(
        talentSource["estilo_base"] ??
          talentSource["base_style"] ??
          talentSource["estilo"],
      ) ||
      normalizeStyleName(
        dominantProfile.estilo ?? reportResultado.perfil_dominante,
      ),
    identifiedTalent: asString(
      talentSource.talento_identificado ??
        talentSource.talento ??
        talentSource.descricao_legada,
    ),
    generatedValue: asString(
      talentSource.valor_gerado ??
        talentSource.valor ??
        talentSource.descricao ??
        talentSource.texto,
    ),
    idealContexts: contentList(
      talentSource.contextos_ideais ??
        talentSource.contextos ??
        talentSource.cenarios,
      ["contextos_ideais", "contextos", "cenarios"],
    ),
    strategies: contentList(
      talentSource.estrategias_potencializacao ??
        talentSource.estrategias ??
        talentSource.acoes_legadas,
      ["estrategias_potencializacao", "estrategias", "acoes_legadas"],
    ),
    balancePoint: asString(
      talentSource.ponto_equilibrio ??
        talentSource.equilibrio ??
        talentSource.ponto_de_equilibrio,
    ),
  };

  const pdi: PdiView = {
    priorityObjectives: Array.isArray(pdiSource.objetivos_prioritarios)
      ? pdiSource.objetivos_prioritarios
          .map((item: unknown) => ({
            objective: asString(
              (item as ObjectRecord).objetivo ??
                (item as ObjectRecord).texto ??
                (item as ObjectRecord).title,
            ),
            expectedBenefit: asString(
              (item as ObjectRecord).beneficio_esperado ??
                (item as ObjectRecord).beneficio ??
                (item as ObjectRecord).resultado ??
                (item as ObjectRecord).descricao,
            ),
          }))
          .filter((item) => item.objective.length > 0)
      : [],
    actionPlan: Array.isArray(pdiSource.plano_acao)
      ? pdiSource.plano_acao
          .map((item: unknown) => ({
            action: asString(
              (item as ObjectRecord).acao ??
                (item as ObjectRecord).texto ??
                (item as ObjectRecord).title,
            ),
            frequency: asString(
              (item as ObjectRecord).frequencia ??
                (item as ObjectRecord).periodicidade,
            ),
            indicator: asString(
              (item as ObjectRecord).indicador ?? (item as ObjectRecord).medida,
            ),
            suggestedDeadline: asString(
              (item as ObjectRecord).prazo_sugerido ??
                (item as ObjectRecord).prazo,
            ),
          }))
          .filter((item) => item.action.length > 0)
      : [],
    evolutionIndicators: contentList(
      pdiSource.indicadores_evolucao ??
        structured.indicadores_evolucao ??
        source.indicadores_evolucao,
      ["indicadores_evolucao"],
    ),
    developmentCommitment: asString(
      pdiSource.compromisso_desenvolvimento ??
        structured.compromisso_desenvolvimento ??
        source.compromisso_desenvolvimento,
    ),
  };
  ...
  return {
    ...
    status:
      ...
      list.length ||
      firstSteps.length ||
      pdi.priorityObjectives.length ||
      pdi.actionPlan.length ||
      pdi.evolutionIndicators.length ||
      pdi.developmentCommitment
        ? "generated"
        : "insufficient_evidence",
    enabled: source.enabled !== false,
    list,
...
function buildRanking(
  report: any,
  reportData: ObjectRecord,
): ReportV36ViewModel["ranking"] {
  const assessment = pickObject(
    report?.assessment,
    reportData?.resultado,
    report?.report_data?.resultado,
  );
  const rawRanking = Array.isArray(report?.ranking)
    ? report.ranking
    : Array.isArray(assessment.ranking)
      ? assessment.ranking
      : [];
  if (rawRanking.length > 0) {
    return rawRanking.map((item: unknown, index: number) => {
      const record = asRecord(item);
      const style = normalizeStyleName(
        record.style ?? record.estilo ?? record.nome ?? record.title,
      );
      const points = toNumber(
        record.score ?? record.points ?? record.pontos ?? record.pontuacao,
      );
      const percentage = toNumber(record.percentage ?? record.percentual);
      return {
        position:
          toNumber(
            record.position ?? record.posicao ?? record["posição"] ?? index + 1,
          ) || index + 1,
  ...
  const ordered = [
    {
      ...
      style: "Assertivo",
      points: toNumber(scores.Assertivo ?? scores.assertivo),
      position: 1,
    },
    {
      role: "Auxiliar" as const,
      style: "Participativo",
      points: toNumber(scores.Participativo ?? scores.participativo),
      position: 2,
    },
    {
      role: "Terciário" as const,
      style: "Integrador",
      points: toNumber(scores.Integrador ?? scores.integrador),
      position: 3,
    },
    {
      role: "Adjacente" as const,
      style: "Analítico",
      points: toNumber(scores.Analitico ?? scores.analitico),
      position: 4,
    },
  ...
  return ordered
    ...
    .map((item, index) => ({
      ...item,
      position: index + 1,
      percentage: Math.round((item.points / total) * 100),
    }));
}

function extractV30Field(
  report: any,
  fieldId: V30ReportFieldName,
): ObjectRecord {
  const publicReport = getReportOutputPublic(report);
...
export function buildReportV36ViewModel(response: unknown): ReportV36ViewModel {
  ...
  const reportData = pickObject(
    root.report_data,
    rawPayload.report_data,
    getFullReportData(root),
  );
  ...
  const v30Report =
    normalizedReport.declared &&
    normalizedReport.validation.valid &&
    normalizedReport.validation.report
      ? normalizedReport.validation.report
      : null;
  ...
  const publicField = (
    fieldId: V30ReportFieldName,
    title: string,
    legacyFallback: unknown = null,
  ): ReportTextFieldView => {
    const source = extractV30Field(root, fieldId);
    const fallback =
      legacyFallback ?? reportResultado[fieldId] ?? reportData[fieldId];
  ...
  return {
    identification: {
      name: asString(
        identification.nome ?? identification.userName ?? identification.name,
      ),
      company: asString(
        identification.empresa ??
          identification.companyName ??
          identification.company,
      ),
      generatedAt: asString(
        identification.generated_at ??
          identification.generatedAt ??
          identification.completedAt,
      ),
      reportUuid: asString(
        root.relatorio_uuid ??
          root.relatorioUuid ??
          root.id ??
          sourceReport?.relatorioUuid,
      ),
    },
    ranking: resultRanking,
    fields: {
      predominant: publicField(
        "perfil_predominante",
        "Perfil Predominante",
        reportResultado.perfil_dominante ?? reportData.perfil_dominante,
      ),
      secondary: publicField(
        "perfil_secundario",
        "Perfil Secundário",
        reportResultado.perfil_secundario ?? reportData.perfil_secundario,
      ),
      light: publicField(
        "lado_luz",
        "Quando você atua no seu melhor",
        reportDinamica.lado_luz ?? reportData.lado_luz,
      ),
      shadow: publicField(
        "lado_sombra",
        "Quando o excesso pode limitar seus resultados",
        reportDinamica.lado_sombra ?? reportData.lado_sombra,
      ),
      development: publicField(
        "estilo_a_desenvolver",
        "Estilo a Desenvolver",
        reportDinamica.estilo_a_desenvolver ?? reportData.estilo_a_desenvolver,
      ),
    ...
    ragAudit:
      reportAuditoria.trilha_rag ??
      reportData.auditoria ??
      reportData.chunk_content_audit ??
      null,
  };
}
